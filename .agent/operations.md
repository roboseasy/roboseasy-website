...
- **제품 id 변경 금지**: URL(`/products/<id>`)이 바뀜
- **스키마 이중 관리**: `src/data/products.ts` 타입 ↔ `public/admin/config.yml` 필드를 함께 수정해야 함
...
- 문의 기록의 원본은 **수신 메일함** — 첨부(사업자등록증)는 서버에 저장되지 않음 (DB 없음)
- 보유기간 운영 규칙: 일반 문의 1년 / 불만·분쟁 3년 (개인정보처리방침 제3조) — 메일함 정리 시 준수
- **Resend 무료 100/day 한도**: 초과하면 발송이 실패한다. 문의가 늘면 유료 전환 검토

## 테스트

> 아직 자동화된 테스트 없음. 문의 폼·견적서 다운로드는 로컬(`npm run dev`)에서 수동 확인.
