...
| `SUPABASE_URL` / `SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY` | 백엔드 — anon 키는 유저 토큰 클라이언트(RLS 적용) 생성용, service role은 RLS 우회 작업 전용·클라이언트 노출 금지. **빌드 시 products 동기화(sync-products)도 사용** | backend.md §1 |
| `CRON_SECRET` | 스케줄러 전용 엔드포인트(`/api/v1/cron/*`) 인증 토큰. Supabase Vault의 `cron_secret`과 동일 값으로 설정 | 마케팅 재확인 배치가 401로 거부됨 |
| `TOSS_SECRET_KEY` | 토스 결제 승인·취소 API(서버 전용 — 클라이언트 노출 금지) | 결제 승인·PAID 주문 취소가 "서버 설정 오류" 500 |
| `PUBLIC_TOSS_CLIENT_KEY` | 토스 결제위젯(클라이언트 키 — `PUBLIC_` 접두사로 번들에 인라인됨, 클라이언트 키만 사용) | 주문서에서 결제 모듈 로드 실패 안내 |
